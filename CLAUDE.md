...
- The header reproduces the **printed BITAC stationery** and must stay that way: national emblem left, BITAC gear right, and five centred lines — centre name (19pt, **violet `#5b2d90`**), শিল্প মন্ত্রণালয় (11pt, **red `#c00000`**), গণপ্রজাতন্ত্রী বাংলাদেশ সরকার, the address, then `ফোন: … , ওয়েবসাইট : …`. **No rule under the header, no English caption line.** The three inks are consts (`TITLE_INK` / `MINISTRY_INK` / `BODY_INK`) — they're features of the stationery, not per-centre branding.
- ⚠️ **Address + contacts belong in the HEADER**, matching the real letterhead. The footer is page numbers only — don't put them back in both. `margin_top` is 44mm to clear the five lines; shrinking the header means re-checking it.
- The website is Latin inside a Bangla line, so it's wrapped in a `dejavusans` span (`contactLine()`) or mPDF substitutes glyphs.
- Everything else is **per-centre data** on `centers` (`name_bn`, `ministry_bn`, `government_bn`, `address_bn`, `phone_bn`, `website`, `logo_left_path` = emblem, `logo_right_path` = gear), editable under **Admin → Centers**. `letterhead_color` is no longer used by the letterhead; `caption_en` now only feeds the public portfolio site.
