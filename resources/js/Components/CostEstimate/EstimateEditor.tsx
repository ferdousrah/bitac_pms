...
export const newLine = (section: Line['section']): Line => ({
...
/**
 * The cost-estimate editing surface — header, the four cost sections with
 * their catalogue pickers, weight/HT calculators and AI rate suggestions,
 * the cost summary and notes.
 *
 * It is controlled: pass `data` and an Inertia-style `setData` (key/value or
 * whole object). The single-estimate page drives it from `useForm`; the Job
 * Costing editor drives one per part from its own state. `setData` MUST be
 * safe to call several times in a row (functional updates) — e.g. changing
 * the pricing group sets the group and then re-prices every line.
 */
interface EstimateEditorProps {
    data: any;
    setData: (keyOrData: any, value?: any) => void;
    errors?: Record<string, any>;
    materials: any[];
    operations: any[];
    customers?: any[];
    /** Hide the customer/company row when it's shared context (job editor). */
    hideCustomer?: boolean;
    /** Rendered under the Notes field — the page's own save buttons go here. */
    footer?: ReactNode;
}

export default function EstimateEditor({
    data, setData, errors = {}, materials, operations, customers = [], hideCustomer = false, footer,
}: EstimateEditorProps) {
