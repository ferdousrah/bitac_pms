...
export default function MaterialCategoryCreateEdit({ category }: any) {
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        if (isEdit) put(`/admin/material-categories/${category.id}`);
        else        post('/admin/material-categories');
    };

    return (
        <AppLayout header={isEdit ? 'Edit Material Category' : 'New Material Category'}>
                    ...
                    <div className="card-header">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500 to-teal-600 flex items-center justify-center text-white">
                                <i className={`fi fi-rr-${isEdit ? 'pencil' : 'cube'} text-lg`} />
                            </div>
                            <div>
                                <h2 className="text-lg font-bold text-surface-900">
                                    {isEdit ? 'Edit Material Category' : 'New Material Category'}
                                </h2>
                                <p className="text-sm text-surface-500">
                                    Used to group materials in the Materials master + Cost Estimates
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div className="form-group sm:col-span-2">
                                    ...
                                    <input type="text" value={data.name}
                                        ...
                                        className="form-input" placeholder="e.g. Cast Iron" required maxLength={100} />
                                    {errors.name && <p className="form-error">{errors.name}</p>}
                                </div>
                                <div className="form-group">
                                    <label className="form-label">Code <span className="form-label-optional">auto if blank</span></label>
                                    <input type="text" value={data.code}
                                        onChange={e => setData('code', e.target.value)}
                                        className="form-input font-mono" placeholder="cast_iron" maxLength={50} />
                                    <p className="form-hint">Stable identifier — keep snake_case</p>
                            ...
                            <div className="flex items-center gap-3 pt-4 border-t border-surface-200">
                                <button type="submit" disabled={processing} className="btn-primary">
                                    {processing ? <><i className="fi fi-rr-spinner animate-spin mr-1.5" /> Saving...</>
                                                 : <><i className={`fi fi-rr-${isEdit ? 'check' : 'plus'} mr-1.5`} /> {isEdit ? 'Update Category' : 'Create Category'}</>}
                                </button>
                                <Link href="/admin/material-categories" className="btn-ghost">Cancel</Link>
                            </div>
                        </form>
                    </div>
                </div>
